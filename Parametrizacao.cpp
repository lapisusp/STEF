...
// Outputs: Knot vector
//----------------------------------------------------------------
void Parametrizacao::curv_parametrize(GLint numpts, GLfloat datpts[][3],
		GLint opcl, GLint param, GLfloat knots[]) {
	...
	if (opcl == OPEN) {
		...
		for (i = 2; i >= 0; i--) {
			knots[i] = knots[i + 1]
					+ (knots[numpts + i - 1] - knots[numpts + i]);
		}

		for (i = 3; i <= 5; i++) {
			knots[numpts + i] = knots[numpts + i - 1]
					+ (knots[i + 1] - knots[i]);
		}
	}
...
//--------------------------------------------------------------------
GLfloat Parametrizacao::curv_pt_dist(GLint num1, GLfloat first[][3], GLint num2,
		GLfloat last[][3]) {
